...
var grid =
{
    ...
    _KEY_CONTAINER_STYLE_  : 'key4containerStyle',   // Key used to store the style of a sub-grid container widget
    ...
    getContents : function()
    {
        ...
        for( const widget of aSave )
        {
            ...
            if( widget.subGrid )
            {
                ...
                const containerStyle = $card.data( this._KEY_CONTAINER_STYLE_ );
     ...
     * - If the card is a sub-grid container, opens the container style dialog.
     * - If the card holds a gadget, opens the gadget properties dialog.
     *
     * @returns This instance.
     */
    editCard : function()
    ...
    showHelp : function()
    {
        let sHTML = '<table class="table is-striped">'+
                        ...
                        '<tbody>'+
                            ...
                            '</tr>'+
                            '<tr>'+
                                '<td>F2</td><td>Open properties editor for selected card</td>'+
                            '</tr>'+
                            '<tr>'+
    ...
    _loadSubGridCard_ : function( card )
    {
        ...
        {
            $(parentEl).data( this._KEY_CONTAINER_STYLE_, card.containerStyle );

            const $frame  = $(parentEl).find('> .grid-stack-item-content > .container-style-frame');
            const adapter = { style       : card.containerStyle,
                              getContainer: () => $frame };

            new GadgetStyle( adapter );
        }
    ...
    _cloneSubGridCard_ : function()
    {
        ...
        // Calculate bottom position so the clone appears last
        let maxY = 0;
        ...
        this._addContainerFrame_( newParent );

        // Copy container style from source to the clone
        const srcStyle = $(sourceEl).data( this._KEY_CONTAINER_STYLE_ );

        if( srcStyle )
        {
            const clonedStyle = JSON.parse( JSON.stringify( srcStyle ) );

            $(newParent).data( this._KEY_CONTAINER_STYLE_, clonedStyle );

            const $frame  = $(newParent).find('> .grid-stack-item-content > .container-style-frame');
            const adapter = { style       : clonedStyle,
                              getContainer: () => $frame };
    ...
    _cleanEmptySubGrids_ : function()
    {
        ...
        for( const el of mainItems )
        {
            ...
            if( node.subGrid.getGridItems().length === 0 )
            {
                ...
                // Clear the content area and any stored container style
                $(el).find('.grid-stack-item-content').empty();
                $(el).removeData( this._KEY_CONTAINER_STYLE_ );
            }
        }
    },

    /**
     * Inserts a `.container-style-frame` div inside the `.grid-stack-item-content` of
     * the given card element. The frame is absolutely positioned over the content area
     * and is used to render user-defined borders without being overridden by the focus
     * indicator class applied to `.grid-stack-item-content` itself.
     *
     * @param {HTMLElement} el A `.grid-stack-item` element.
     * @returns {jQuery} The newly created frame element.
     */
    ...
    /**
     * Opens a style-only properties dialog for a sub-grid container card.
     * The dialog reuses the existing #properties-style panel, bound to a lightweight
     * adapter that satisfies the GadgetStyle / dlgStyle interface without a real gadget.
     */
    _editContainerStyle_ : function()
    {
        const parentEl   = this._$focused_[0];
        const $frame     = $(parentEl).find('> .grid-stack-item-content > .container-style-frame');
        const savedStyle = $(parentEl).data( this._KEY_CONTAINER_STYLE_ ) || {};

        // Adapter: presents the same interface that GadgetStyle and dlgStyle expect from a gadget
        const adapter = { style       : savedStyle,
                          getContainer: () => $frame };

        $('<div></div>')
            .append( $('#properties-style').show() )
            .dialog( {
                title    : 'Container Style',
                modal    : true,
                autoOpen : true,
                resizable: true,
                width    : p_app.getBestWidth( '90%', 900, 700 ),
                open     : function()
                           {
                               setTimeout( function() { dlgStyle.setup( adapter ); }, 500 );
                           },
                beforeClose: function()
                           {
                               $(parentEl).data( grid._KEY_CONTAINER_STYLE_, adapter.style );
                               grid._isChanged_ = true;
                               return true;
                           },
                close    : function()
                           {
                               $('#properties-style').hide();
                           }
            } );
    },

    _onContextMenu_ : function( cardEl, e )
    {
        this._setFocus_( cardEl, e );

        gt.showContextMenu( e.pageX, e.pageY,
            [
                { label: 'Properties [F2]',  icon: 'settings',  action: () => grid.editCard()  },
                { label: 'Clone [Ins]',      icon: 'copy',      action: () => grid.cloneCard() },
                { label: 'Delete [Del]',     icon: 'trash',     action: () => grid.delCard()   }
            ] );
    },
