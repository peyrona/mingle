...
var gum_ws =
{
    ...
    connect : function( fnOnConnected )
    {
        ...
        else
        {
            ...
            parentContext = this._getContext_( context.parentContextId );
            if( parentContext && parentContext.socket )
            {
                // Parent exists and has valid socket - share it
                context.socket = parentContext.socket;
                shouldConnect = false; // Parent's socket should already be connected
            }
            else
            {
        ...
        if( context.socket )
        {
            context.socket.addOnMessage( (msg) => {
                                                    // Only process messages for current context to prevent cross-contamination
            ...
            context.socket.addOnError( (error) =>
                                    {
                                        ...
                                        if( error.target && error.target.readyState === WebSocket.CLOSED )
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                this._cleanupSessionStorage_( isDesign );
                                            }
                                        }
                                    } );
    ...
    close : function()
    {
        ...
        this._cleanupSessionStorage_( gum.isInDesignMode() );
    },
     ...
     * But when sending Une advanced types (date, time, list, pair) it should be present. If
     * it is not, Arrays are sent as Une 'list' and Objects as Une 'pair'.
     *
     * If omitted, if value is a string and its content is "true" or "false", value will be converted
     * into boolean. Same if the string contains a valid number: value is converted into Number.
     *
     * @param {Object} oExEnAddr   ExEn address. E.g.: {"host":<str>,"port":<number>,"ssl":boolean}
     ...
     * <br>
     * Device names can be a single string or an array of strings.
     * <br>
     * Note: when deviceName ends with '*' matches all devices which name start with previous to asterisk chars.
     *       e.g. "control_*" matches "control_A1", "control_A2", ...
     *
     ...
     * @param {String} sAction Which action or actions the listener is interested in ("readed", "changed", ...). Defaults to '*'.
    ...
    _onMessageReceived_ : function( msg )      // msg is like this: { "exen": { host: 192.168.1.7, port: 55880, ssl: false }, "Changed": {"when":1659613508344, "name":"clock", "value":1521284 } }"
    {
        ...
        switch( sAction )
        {
            ...
            // default: --> e.g. 'read' (to read the value of a device) can arrive here
    ...
    /**
     * Cleans up sessionStorage based on mode.
     *
     * @param {boolean} isDesign True for design mode, false for preview mode
     */
    _cleanupSessionStorage_ : function( isDesign )
    {
        if( isDesign )
    ...
    _areSameName_ : function( nameListener, nameEvent )
    {
        ...
        if( p_base.isString( nameListener ) && p_base.isString( nameEvent ) && nameListener.endsWith( '*' ) )   // e.g. 'light_*'
