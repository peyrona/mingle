...
var grid =
{
    _KEY_GADGET_  : 'key_gadget',   // Key used to store a gadget in the 'data' of a widget (a <div>)
    ...
    editCard : function()
    {
        ...
        const widget = this._lastFocused_[0];
        const id     = widget.id;
    },
    ...
    addGadget : function( gadget )
    {
        ...
        this._lastFocused_.find('.grid-stack-item-content')
                          ...
                          .data( this._KEY_GADGET_, gadget )    // Store the gadget in the widget
    ...
    _getGadgetById_ : function( id )
    {
        for( const widget of grid.engine.nodes )
        {
            if( widget.id === id )
                return widget.find('div').eq(1);    // 1st is the north title, 2nd is the gadget, 3rd the south title
        }
    ...
    _init_ : function()
    {
        ...
        this._stack_.on('resizestop', (evt, div) =>
                        {
                            const widget = div.gridstackNode;
                            grid._getGadgetById_( widget.id ).show();
                        } );
