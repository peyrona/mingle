...
//----------------------------------------------------------------------------------------------------------//
// Load libs from local HD (async because they are needed to be ready before gum starts)

let asLib = ["/gum/lib/chart_v4.4.1.min.js",    // Do not change the order for chart.js and dependencies
...
console.log( "All local libs were loaded" );
...
var gum =
{
    ...
    _design_   : false,   // Is in design mode? (used only at client: not sent to server)
    ...
    getLastBodySize : function()
    {
        return this._bodySize_;
    },
...
// TODO: hacerlo -> eliminar los gadgets q usen este device y del gum._aoExEn_
    ...
    save : function( fnOnOK = null )
    {
        ...
        let oData = {
                      ...
                      rest: { background: this._oBackGr_,        // Everything else: this will be returned by _template_.html::getConfig()
                              ...
                              body_size : p_app.getBodySize(),   // Current Browser Window size
    ...
    _setButtonBar_()
    {
        ...
        let sOnHome = "window.location='/gum/index.html'";     // Shows initital page (::save() is invoked on page unload)

        let sHTML = '<i class="gum-mini-btn fa fa-home"            title="Go to initial page"   onclick="'+ sOnHome +'"          ></i>'+
                    '<i class="gum-mini-btn fa fa-eye"             title="Preview (in new tab)" onclick="gum._onPreview_()"      ></i>'+
    ...
    _onPreview_ : function()
    {
        gum.save( function()
                    {
                        let loc = gt.normalizeHtmlName( gum._name_ ) +'?design=false';

                        window.open( p_base.doURL( loc ), '_blank' ).focus();
                    }
                );
    },
    ...
    _updateExEns_ : function( axExEn, fnOnDone )
    {
        ...
        {                                                                          // '.replace(...)' removes extra '\'
            ...
            gum_ws.requestList( this._aoTarget_.at( -1 ).exen,    // at( -1 ) -> Get last item
                                function( oExEn, aoDevs )
                                {
                                    let oTarget = gum.getTarget4( oExEn );

                                    if( oTarget === null )
                                        throw "Error: this should not happen";

                                    aoDevs.forEach( (oDev) => { if( oDev.cmd === 'device' ) oTarget.devs.push( oDev ); } );
    ...
    _init_ : function()
    {
        ...
        let name    = window.location.pathname.split('/').pop();
            name    = name.substring( 0, name.length - 5 );      // Removes ".html"
        ...
        // Init internal design mode and layout type
        let urlParams = new URLSearchParams( window.location.search );

        this._design_ = (urlParams.get('design') === 'true');
        ...
        gum_ws.connect( () =>    // On successfully connected via WebSockets with the server
            {
                ...
                gum._bodySize_ = oConfig.body_size;
