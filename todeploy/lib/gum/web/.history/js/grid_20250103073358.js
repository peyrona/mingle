...
var grid =
{
    ...
    addCard : function( card = null )     // null == add new card
    {
        ...
        this._setFocus_( widget );     // Must be before 'addGadget' because it uses '_focused_'
    ...
    editCard : function()
    {
        ...
        const gadget = this._focused_.data( this._KEY_GADGET_ );

        if( gadget )      // 'if' because the widget could be empty or could be no card focused
        {
            gadget.fnOnEditEnd = this._moveTitleFromGadgetToCard_;
            gadget.edit( true );
        }
    ...
    delCard : function()
    {
        ...
        this._stack_.removeWidget( this._focused_[0] );
        this._isChanged_   = true;
        this._focused_ = null;
    ...
    addGadget : function( gadget )
    {
        ...
        this._focused_.data( this._KEY_GADGET_, gadget )    // Store the gadget in the widget (has to be 1st because 'find(...) crerates a new object)
    ...
    _setFocus_ : function( card )
    {
        ...
        if( this._focused_ )
            this._focused_.find('.grid-stack-item-content').removeClass('grid-widget-focus');

        this._focused_ = $(card);
        this._focused_.find('.grid-stack-item-content').addClass('grid-widget-focus');

        this._updateButtonsState_();
    },

    _moveTitleFromGadgetToCard_ : function( gadget )
    {
        let title = gadget['card_title'];
        let locat = gadget['card_title_location'];

        if( gadget.hasOwnProperty( 'card_title' ) )
            delete gadget['card_title'];

        if( gadget.hasOwnProperty( 'card_title_location' ) )
            delete gadget['card_title_location'];

        let card = this._focused_;

        card['title'] = { text    : title ? title : null,     // 'null' to avoid 'undefined'
                          location: locat ? locat : null };

        this._showTitle_( card );
    },

    _showTitle_ : function( card = this._focused_ )
    {
        if( ! card )
            return;
    ...
    {
        const hasFocusedWidget = this._focused_ !== null;
