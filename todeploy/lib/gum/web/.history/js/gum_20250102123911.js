...
// TODO: hacerlo -> eliminar los gadgets q usen este device y del gum._aoExEn_
    ...
    _setButtonBar_()
    {
        ...
        let sHTML = '<i class="gum-mini-btn fa fa-eye"             title="Preview (in new tab)" onclick="gum._onPreview_()"      ></i>'+
    ...
    _init_ : function()
    {
        ...
        $(window).on('resize', function( event )    // Better not to touch this
                                {
                                    if( free._resizeTimer_ )
                                        clearTimeout( free._resizeTimer_ );

                                    if( event && event.target === window )    // 'window' is the browser window
                                        free._resizeTimer_ = setTimeout( function() { free._onBrowserResized_(); }, 350 );   // Must be 'function', not lambda
                                } );
