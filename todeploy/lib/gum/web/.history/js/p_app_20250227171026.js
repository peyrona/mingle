...
var p_app =
{
    ...
    setSessionTimeout : function( fn, nMinutes = 45 )
    {
        ...
        let millis = parseInt( nMinutes ) * 60 * 1000;
            millis = (millis < 1) ? 1 : millis;

        $(document).on( 'keyup click touch', () => self.nLastUserActionTime = Date.now() );
    ...
    /**
     * Muestra un error AJAX en un ::alert().
     *
     * @param {Object} xhr El error AJAX (el que envía JQuery en las llamadas AJAX).
     * @param {String} sMsg Mensaje opcional para mostrar. By default null.
     * @param {Function} fnOnClosed Function to be invoked after message dialog is closed. Optional.
     */
    showAjaxError : function( xhr, sMsg = null, fnOnClosed = null )
    {
        let sErr408 = "";
        ...
        {
            if( xhr.status === 408 )   // 408 --> request timed out
                sErr408 = "Server not avaibale.<br>Please check your Internet connection and try again.<br><br>";

            let msg = ((! xhr) ? "Unknown" : xhr.responseText );

            if( p_base.isEmpty( msg ) )
                msg = xhr.statusText;

            if( p_base.isNotEmpty( sMsg ) && (! p_base.isString( sMsg )) )
                sMsg = String( sMsg );

            msg = sErr408 + (p_base.isNotEmpty( sMsg ) ? sMsg +"<br>"+ msg : msg);

            if( p_base.isEmpty( msg ) )
                msg = "Unknowm error";

            this.alert( msg, "Ajax error", fnOnClosed );
        }
    },
