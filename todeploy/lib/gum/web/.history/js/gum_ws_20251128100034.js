...
var gum_ws =
{
     ...
     */
    createContext : function( dashboardId, isDesign, parentContextId = null )
    {
        const context = {
     ...
     */
    getContext : function( dashboardId )
    {
     ...
     */
    setCurrentContext : function( dashboardId )
    {
    ...
    connect : function( fnOnConnected )
    {
        ...
        if( ! currentContext || currentContext.socket !== null )
            return;
        ...
        // Create or get context for this dashboard
        const dashboardId = tabId; // Use tabId as dashboard ID for context management

        let context = this.getContext( dashboardId );

        if( ! context )
        {
            context = this.createContext( dashboardId, isDesign, isDesign ? null : tabId );
        }
        else
        {
            this.setCurrentContext( dashboardId );
        }
        ...
        else
        {
            ...
            const parentContext = this.getContext( context.parentContextId );
            ...
            {
                // Fallback: create own connection if parent not found
                const wsUrl = "ws://" + location.host + "/gum/bridge/" + tabId;
                context.socket = new WebSocketWrap( p_base.addCacheBuster( wsUrl ) );
            }
        }
        ...
        context.socket.addOnMessage( (msg) => {
            ...
            if( this.getCurrentContext() && this.getCurrentContext().dashboardId === dashboardId )
            {
        ...
        if( isDesign || ! this.getContext( context.parentContextId ) )
        {
