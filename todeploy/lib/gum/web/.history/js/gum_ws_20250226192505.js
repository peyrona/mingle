...
var gum_ws =
{
        ...
        this.socket.addOnError( (error) =>
                                {
                                    if( error.target.readyState === WebSocket.CLOSED )
                                    {
                                        ...
                                        // Error message
                                        const errorThrown = 'Unauthorized';

                                        // Trigger the ajaxError event manually with proper parameters
                                        $(document).trigger('ajaxError', [jqXHR, ajaxOptions, errorThrown]);


                                        // let url = location.href.substring( location.origin.length );
                                        // window.location.href = '/gum/login.html?returnUrl='+ encodeURIComponent( url );
                                    }
                                    else
                                    {
                                        p_app.alert( JSON.stringify( error ), "WebSocket Error" );
                                    }
                                }
                              );
     ...
     * @param {String|Array} sDevNames One or more (an array) device name(s) the listener is interested in ('*' can be used).
     ...
     */
    addListener : function( oExEn, sDevNames, fnCallback, sAction = '*' )
    {
        if( p_base.isString( oExEn ) )
            oExEn = JSON.parse( oExEn.replace( /\\"/g, '"' ) );    // This 'replace(...)' removes extra '\'

        let sUUID   = p_base.uuid();
        let devices = p_base.isArray( sDevNames) ? sDevNames : [sDevNames];
    ...
    _onMessageReceived_ : function( msg )      // msg is like this: { "exen": { host: 192.168.1.7, port: 55886, ssl: false }, "Changed": {"when":1659613508344, "name":"clock", "value":1521284 } }"
    {
        ...
        switch( sAction )
        {
            ...
            case 'change':                                       // Nothing to do: I do not see this is going to used
                break;
