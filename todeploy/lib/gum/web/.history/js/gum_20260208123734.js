...
var gum =
{
    ...
    run : function( oConfig )    // This function is called by _template_.html::getConfig() to pass the config
    {
        this._loadAllScripts_()
            ...
            {
                let msg = 'jqxhr: '+ jqxhr +'/nsettings: '+ settings +'/nexception:'+ exception;
                console.error('Error during initialization:', msg);
...
// TODO: hacerlo -> eliminar los gadgets q usen este device y del gum._aoExEn_
    ...
    _setButtonBar_()
    {
        ...
        let $select = $('<select>' +
                            ...
                            '<option value="scheduler">Scheduler (Kronos)     </option>' +
                        '</select>');
    ...
    _updateExEns_ : function( axExEn, fnOnDone )
    {
        ...
        gum_ws_boards.setOnList( (oExEn, aoDevs) =>     // Even if this call has to be done only once, it is done here just for clarity
                {
    ...
    _save_ : function( fnOnOK = () => {} )
    {
        ...
        $('#gum-toolbar').find('.ti-device-floppy')
                         .css('color', 'red');
        ...
        window.addEventListener('beforeunload', function( evt )
            {
                ...
                {
                    evt.preventDefault();
                    evt.returnValue = 'Dashboard is currently saving. Please wait a moment and try again.';
                    return;
                }

                if( gum.isInDesignMode() && gum._layout_.isChanged() )  // Don't need to worry about Dashboard configuration because the changes are saved after closing the dialog
                    evt.preventDefault();
            } );
...
// FIXME: --->
        ...
        // Create context for this dashboard before connecting
        let dashboardId;
        ...
        if( gum.isInDesignMode() )    // Design mode: generate unique dashboard ID
        {
            ...
            if( ! dashboardId || dashboardId.length < 5 )
            {
                dashboardId = p_base.uuid();
                sessionStorage.setItem('gum_tab_id_design', dashboardId);
            }
        }
        else   // Preview mode: inherit parent dashboard ID from URL parameter
        {
            const urlParams = new URLSearchParams( window.location.search );
            dashboardId = urlParams.get('parentTabId');

            if( ! dashboardId )
            {
                // Fallback: generate own ID if parentTabId not found
                dashboardId = sessionStorage.getItem('gum_tab_id_preview');
                if( ! dashboardId || dashboardId.length < 5 )
                {
                    dashboardId = p_base.uuid();
                    sessionStorage.setItem('gum_tab_id_preview', dashboardId);
                }
            }
        }
    ...
    _loadDesignChunks_ : function()
    {
        const deferred      = $.Deferred();
        let   nLoadCounter  = 0;
        const nChunksToLoad = 10;
