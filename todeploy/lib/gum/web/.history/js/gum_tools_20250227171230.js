...
if( typeof authenticator === "undefined" )
{
    var authenticator =
    {
        ...
        start : function( options )
        {
            ...
            $('#auth-login-form').on( 'submit', (event) => this._handleLogin_(event) );

            // Intercepts AJAX errors to show the dialog
            $(document).ajaxError( (event, xhr) => authenticator.doDefaultOnAjaxError( xhr ) );

            // Forces a logout after the timeout specified in the server
            $.ajax( {   url     : '/ws/util/sessionTimeout',
                        method  : 'GET',
                        error   : (xhr)  => p_app.setSessionTimeout( this._doSessionExpired_, 30 ),
                        success : (secs) => p_app.setSessionTimeout( this._doSessionExpired_, parseInt( Number(secs)/60 ) )
                    } );
