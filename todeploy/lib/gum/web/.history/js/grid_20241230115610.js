...
{
    $.ajax( {
        url     : "/gum/lib/gridstack_v7.2.3.all.js",
        dataType: 'script',
        async   : false,
        success : function() { console.log( "Loaded: GridStack" ); },
    } );

    $('div-grid').addClass('grid-stack');    // Needed for GridStack to identify its <DIV>
...
var grid =
{
    ...
    _init_ : function()
    {
        ...
        $('#gum-toolbar').append( $('<br>'+
                                    '<div>'+
                                        '<i class="gum-mini-btn fa fa-plus"   title="Insert a card above"  onclick="grid.add()"></i>'+
                                        '<i class="gum-mini-btn fa fa-trash"  title="Insert a card below"  onclick="grid.del()"></i>'+
                                    '</div>') );
    }
};
}