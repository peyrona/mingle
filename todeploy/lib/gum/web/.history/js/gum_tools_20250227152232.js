...
var gt =
{
    ...
    class Authenticator {
        constructor(options) {
            const { container, title, success } = options;

            if (container.charAt(0) !== '#') {
                throw new Error("Container must start with '#'");
            }
        ...
        // Method to inject the login dialog HTML into the container
        injectLoginDialog() {
            const dialogHTML = this.getDialogHTML().replace('{*Title*}', this.title);
            $(this.container).html(dialogHTML);
        }

        // Method to generate the login dialog HTML
        getDialogHTML() {
            return `
        ...
        handleLogin(event) {
            ...
            const username = $('#auth-login-form input[type="text"]:first').val();
            const password = $('#auth-login-form input[type="password"]:first').val();
            ...
            $.ajax({
                ...
                error: (xhr) => {
                    if (xhr.status === 401) {
                        ...
                        // Assuming p_app.showAjaxError is a global function
        ...
        // Method to attach the login form submission handler
        attachLoginHandler() {
            $('#auth-login-form').on('submit', (event) => this.handleLogin(event));
        }

        // Method to intercept AJAX errors and handle 401 errors
        interceptAjaxErrors() {
