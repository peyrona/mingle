...
class GumCheck extends GumGadget
{
    constructor( props )
    {
        ...
        if( this.exen && this.device )
            gum_ws.requestValue( this.exen, this.device );
    }
    ...
    _updateListener_()
    {
        ...
        if( this.device !== null )
        {
            ...
            let fn = function( action, when, name, value )
                    {
                        // if( self._getReadedCounter( action ) > 0 )   --> Not used by check
                        //     return;

                        if( ! p_base.isBoolean( value ) )
                        {
                            p_app.alert( 'Device "'+ name +'" is not Boolean; its value is: '+ value );
                        }
                        else
                        {
                            ...
                            $('#'+self.id).attr('src', (value ? sOn : sOff) );
                        }
                    };
