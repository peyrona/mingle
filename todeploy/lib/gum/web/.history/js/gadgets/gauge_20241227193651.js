...
class GumGauge extends GumGadget
{
    ...
    show( isOngoing = false )
    {
        ...
        this.getContainer().attr('width' , Math.max( this.getContainer().width() , 48 ))
            .attr('height', Math.max( this.getContainer().height(), 48 ));

        this._refreshWindow_();
    ...
    /**
     * Adapts the size of the window to the size of its content pane (only when in desig mode).
     *
     * @returns Itself.
     */
    _refreshWindow_()
    {
        ...
        let $window = this.getContainer().parent();

        $window.attr('width' , this.getContainer().width()  + 6 )
               .attr('height', this.getContainer().height() + 9 );
    ...
    _updateListener_()
    {
        ...
        if( this.device !== null )
        {
            ...
            let fn = function( action, when, name, value )
                    {
                        if( self._getReadedCounter( action ) > 0 )
                            return;

                        if( p_base.isNumber( value ) )  self.wrapper.set( value );
