...
var gt =
{
    ...
    {
        // if( gt.read( 'mingle_auth_approved' ) )    // Needed because 'location.reload()' will invoke this function again
        //     return;

        const { container, title, onok: success } = options;
        ...
        function handleLogin( event )      // Function to handle login form submission
        {
            ...
            $.ajax( {
                        ...
                        success    : function(response)
                                    {
                                        $(container).hide();
                                        gt.write( 'mingle_auth_approved', true, -1 );
                                        //location.reload();
                                        success( response );
                                    },
                        error: function(xhr)
        ...
        $(container).html( gt.authGetDialogHTML().replace( '{*Title*}', title ) );

        // Attach the login form submission handler
        $('#auth-login-form').on('submit', handleLogin);
