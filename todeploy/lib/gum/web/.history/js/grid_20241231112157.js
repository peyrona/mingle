...
var grid =
{
    ...
    getContents : function()
    {
        ...
        for( const widget of aSave )    // This 'widget' has minium information (it is not the live instance)
        {
            ...
            let title  = card.el.find('#card-title-north').text() +
                         card.el.find('#card-title-south').text();
    ...
    _getCardById_( id )
    {
        ...
        if( element )  return element.gridstackNode;    // This gives GridStack-specific data
        else           return null;
    },
