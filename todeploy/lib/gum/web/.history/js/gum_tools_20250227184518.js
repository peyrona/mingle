...
if( typeof authenticator === "undefined" )
{
    var authenticator =
    {
        ...
        {
            if( xhr.status === 401 )  $(this.container).show();
            else                      p_app.showAjaxError( xhr );
        },
        ...
        _doSessionExpired_ : function()
        {
            $.ajax( {
                        ...
                        error   : (xhr) => p_app.showAjaxError( xhr ),
                        success : () => {
                                          p_app.alert( "Due to inactivity, session expired.\nYou have to login again.", "Session expired");
        ...
        {
            // "width:100%; max-width:400px; padding:2rem; border-radius:8px; box-shadow:0 2px 10px rgba(10, 10, 10, 0.3); background-color:white;"

            return `
                <div style="display:flex; min-height:100vh; justify-content:center; align-items:center;">
                    <div style="position:fixed; top:50%; left:50%; transform:translate(-50%,-50%); width:100%; max-width:400px; padding:2rem; border-radius:8px; box-shadow: 0 2px 10px rgba(10, 10, 10, 0.3); background-color:#F0F8FF; z-index:1000;">
