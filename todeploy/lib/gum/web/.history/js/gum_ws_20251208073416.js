...
var gum_ws =
{
     ...
     */
    connect: function( bPlain, fnOnConnected )
    {
        if( bPlain )  this._connectPlain_( fnOnConnected );           // Plain connection (no gum) for external use
        else          this._connectForDashboards_( fnOnConnected );   // 'gum' is defined only in Dashboards
    },
    ...
    _connectForDashboards_ : function( fnOnConnected )
    {
        const isDesign = gum.isInDesignMode();

        // Resolve the Shared ID (The Dashboard's ID)
...
class VirtualSocket
{
    ...
    connect()
    {
        // Virtual connection is instant
        console.log("[VirtualSocket] Linked to BroadcastChannel");
    }

    send(data)
    {
        // Send to Leader to forward to Server
