...
var free =
{
    ...
    addGadget : function( gadget )     // Used when in design mode
    {
        ...
        $outter.width(  gadget.width )
               ...
               .draggable( { drag : function(event,ui) { free._onGadgetDrag_( event, ui, true  ); },
    ...
    {
        if( this._aGadgetsSelected_.length === 0 ) return this;
    ...
    {
        const isShift = (eventOrGadget instanceof MouseEvent) && eventOrGadget.shiftKey;
    ...
    _onGadgetDrag_ : function( event, ui, isOngoing )
    {
        ...
        let dx = ui.position.left - ui.originalPosition.left;
        let dy = ui.position.top - ui.originalPosition.top;

        for( const g of this._aGadgetsSelected_ )
        {
            if( g === gadget ) continue; // The dragged one is already handled by jQuery UI
            ...
            if (isOngoing) {
                ...
                $w.offset({ top: $w.data('originalPosition').top + dy, left: $w.data('originalPosition').left + dx });
            } else {
    ...
    _onKeyPressed_ : function( evt )    // Is invoked only when in design mode
    {
        ...
        else if( (evt.keyCode >= 37 ) && (evt.keyCode <= 40) )         // Arrows key codes: left = 37, up = 38, right = 39, down = 40
        {
            ...
            else if( evt.ctrlKey && this._gadgetFocus_.isResizable() )
            {
                ...
                {
                    const $w = gadget.getContainer().parent();
                    if( !gadget.isResizable() ) continue;
    ...
    _showGadgetInfo_ : function( wndOrGad = null )    // Is invoked only when in design mode
    {
        ...
        else
        {
            ...
            let count  = this._aGadgetsSelected_.length;
            let countStr = (count > 1) ? `&nbsp;(${count} selected)` : "";

            $('#free_focused_wnd_info').html( `x=${parseInt( gadget.x )}, y=${parseInt( gadget.y )}, w=${parseInt( width )}, h=${parseInt( height )}, z=${parseInt( gadget.z )} ${countStr}` );
        }
    ...
    _alignLeft_ : function() {
        ...
        this._aGadgetsSelected_.forEach(g => {
            if (g === this._gadgetFocus_) return;
            g.x = targetX;
    ...
    _sameWidth_ : function() {
        ...
        this._aGadgetsSelected_.forEach(g => {
            if (g === this._gadgetFocus_ || !g.isResizable()) return;
            g.width = targetWidth;
