...
var grid =
{
    ...
    editCard : function()
    {
        ...
        if( gadget )      // 'if' because the widget could be empty or could be no card focused
            gadget.edit( true, null, this._updateTitle_() );
    ...
    addGadget : function( gadget )
    {
        ...
        this._lastFocused_.data( this._KEY_GADGET_, gadget )    // Store the gadget in the widget (has to be 1st because 'find(...) crerates a new object)
                          ...
                          .empty()
                          .append( $('<div id="card-title-north" class="card-title" style="hidden"></div>') )
                          .append( gadget.getContainer() )
                          .append( $('<div id="card-title-south" class="card-title" style="hidden"></div>') );
    ...
    _updateTitle_ : function()
    {
        const card   = this._lastFocused_;
        const gadget = card && this._lastFocused_.data( this._KEY_GADGET_ );
        ...
        if( title )
        {
            let where = '#card-title-'+ locat;
            card.find( where ).html( title );
        }
    },
