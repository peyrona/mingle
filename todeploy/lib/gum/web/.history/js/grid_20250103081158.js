...
var grid =
{
    ...
    addCard : function( card = null )     // null == add new card
    {
        ...
        this._setFocus_( widget );     // Must be before 'addGadget' because 'addGadget' uses '_$focused_'
    ...
    _moveTitleFromGadgetToCard_ : function( gadget )
    {
        ...
        {
            title = gadget['card_title'];
            delete gadget['card_title'];
        }

        if( gadget.hasOwnProperty( 'card_title_location' ) )
        {
            locat = gadget['card_title_location'];
            delete gadget['card_title_location'];
        }

        let $card = grid._$focused_;    // Can not be 'this', must be 'grid' bacaue this is the gadget when this function is called from the gadget

        $card[0]['title'] = { text    : title ? title : null,     // 'null' to avoid 'undefined'
                              location: locat ? locat : null };

        grid._showTitle_( $card );      // Can not be 'this', must be 'grid' bacaue this is the gadget when this function is called from the gadget
    },

    _showTitle_ : function( card = this._$focused_ )
    {
        if( ! card )
            return;
