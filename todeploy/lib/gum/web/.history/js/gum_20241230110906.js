...
$.getScript( "https://cdnjs.cloudflare.com/ajax/libs/gridstack.js/7.2.3/gridstack-all.js",
             function() { console.log('Loaded: grid-stack'); } );

//----------------------------------------------------------------------------------------------------------//
// Load libs from local HD (sync because they are needed to be ready before gum starts)
...
let asLib = ["/gum/lib/chart_v4.4.1.min.js",    // Do not change the order for chart.js and dependencies
             ...
             "/gum/lib/ssd.js",
             "/gum/lib/gridstack_v7.2.3.all.js"];
...
for( let n = 0; n < asLib .length; n++ )
{
    $.ajax( {
                ...
                dataType: 'script',
                async   : false,
                success : function() { console.log( "Loaded: "+ asLib[n] ); },
            } );
}
...
// TODO: hacerlo -> eliminar los gadgets q usen este device y del gum._aoExEn_
    ...
    _setButtonBar_()
    {
      ...
      //let sOnHome = "window.location='/gum/index.html'";     // Shows initital page (::save() is invoked on page unload)
        let sOnHome = "window.location='/gum/index.html'";

        let $select = GumGadget.createSelect( oGadget => free._add_( oGadget ) );    // This SELECT has all available gadgets
