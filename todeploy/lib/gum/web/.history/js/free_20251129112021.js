...
var free =
{
            ...
            $('#gum-toolbar').append( $('<br>' +
                                        '<div style="display:flex; justify-content:space-between; align-items:center;">' +
                                            ...
                                            '<span>' +
                                                ...
                                                '<i class="gum-mini-btn fa fa-code"  title="HTML and JavaScript editor" onclick="gum._coder_()" ></i>' +
                                            '</span>' +
                                        '</div>') );
    ...
    _onKeyPressed_ : function( evt )    // Is invoked only when in design mode
    {
        ...
        else if( (evt.keyCode === 33 ) || (evt.keyCode === 34) )       // PgUp = 33, PgDn = 34
        {
            ...
            if( ! p_base.isNumber( this._gadgetFocus_.z ) )
                this._gadgetFocus_.z = 100;    // Default Z index when not defined

            let incr = ((evt.shiftKey || evt.metaKey) ? 10 : 1);

            let z  = this._gadgetFocus_.z;
                z += (evt.keyCode === 33) ? incr : (incr * -1);
                z  = p_base.setBetween( 0, z, p_base.getMaxZIndex() );

            this._gadgetFocus_.z = z;
