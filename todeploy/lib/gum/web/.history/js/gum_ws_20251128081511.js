...
var gum_ws =
{
    ...
    connect : function( fnOnConnected )
    {
        ...
        const mode  = gum.isInDesignMode() ? 'design' : 'preview';
        let   tabId = sessionStorage.getItem('gum_tab_id_' + mode);

        if( ! tabId || tabId.length < 5 )
        {
            tabId = Date.now() + '_' + Math.random().toString(36).substring(2,9);
            sessionStorage.setItem('gum_tab_id_' + mode, tabId);
        }
        ...
        this.socket.addOnError( (error) =>
                                {
                                    ...
                                    if( error.target && error.target.readyState === WebSocket.CLOSED )
                                    {
                                        ...
                                        if( error.wasClean )
                                            ...
                                            sessionStorage.removeItem('gum_tab_id_' + mode);    // Clean up tab ID on abnormal closure
                                    }
                                }
                              );
        ...
        window.addEventListener('beforeunload', () =>   {
                                                            ...
                                                            sessionStorage.removeItem('gum_tab_id_' + mode);
                                                        });
    },
