...
if ($Arch -eq "AMD64") {
    ...
    $JdkArch = "x64"
    $TargetDirName = "jdk.11.windows.x64"
} elseif ($Arch -eq "x86") {
    Write-Host "Detected AMD32 (x86) architecture."
    $JdkArch = "x86"
    $TargetDirName = "jdk.11.windows.x86"
} elseif ($Arch -eq "ARM64") {
    Write-Host "Detected ARM64 architecture."
    $JdkArch = "aarch64"
    $TargetDirName = "jdk.11.windows.aarch64"
} else {
...
$TargetDir = Join-Path $ScriptDir $TargetDirName
