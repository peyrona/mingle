...
// NEXT: this class uses the old date-time Java system. To save RAM (getting rid off this old module), this class has to be redone.
/**
 * Provides methods to determine the sunrise, sunset, astronomical twilight times of a given location, or if it is currently day or night at a given location. <br>
 * Also provides methods to convert between Gregorian and Julian dates.<br>
 * The formulas used by this class are from the Wikipedia articles on Julian Day and Sunrise Equation. <br>
 *
...
final class SunriseSunset
{
    ...
    /**
     * Convert a Gregorian calendar date to a Julian date. Accuracy is to the second.
     * <br>
    ...
    /**
     * Convert a Julian date to a Gregorian date. The Gregorian date will be in the local time zone. Accuracy is to the second.
     * <br>
     * This is based on the Wikipedia article for Julian day.
     *
    ...
    public static Calendar getGregorianDate( final double julianDate )
    {
        ...
        // Ex: 17.208 - 17 = 0.208 days. 0.208*60 = 12.48 minutes. We truncate to 12 minutes.
        final int minutes = (int) ((dayFraction * 24 - hours) * 60d);
        // Ex: 17.208*60 - (17*60 + 12) = 1032.48 - 1032 = 0.48 minutes. 0.48*60 = 28.8 seconds.
     ...
     * @return a two-element Gregorian Calendar array. The first element is the astronomical twilight dawn, the second element is the astronomical twilight dusk. This will return null if there is no astronomical twilight. (Ex: no twilight in Antarctica in December)
     */
     ...
     * @param latitude  the latitude of the location in degrees.
     * @param longitude the longitude of the location in degrees (West is negative)
     * @return a two-element Gregorian Calendar array. The first element is the sunrise, the second element is the sunset. This will return null if there is no sunrise or sunset. (Ex: no sunrise in Antarctica in June)
     * @see <a href="http://en.wikipedia.org/wiki/Sunrise_equation">Sunrise equation on Wikipedia</a>
     */
     ...
     * @param latitude  the latitude of the location in degrees.
     * @param longitude the longitude of the location in degrees (West is negative)
     * @return true if it is currently day at the given location. This returns true if the current time at the location is after the sunrise and before the sunset for that location.
     */
     ...
     * @param latitude  the latitude of the location in degrees.
     * @param longitude the longitude of the location in degrees (West is negative)
     * @return true if it is day at the given location and given datetime. This returns true if the given datetime at the location is after the sunrise and before the sunset for that location.
     */
     ...
     * @param latitude  the latitude of the location in degrees.
     * @param longitude the longitude of the location in degrees (West is negative)
     * @return true if it is night at the given location currently. This returns true if the current time at the location is after the astronomical twilight dusk and before the astronomical twilight dawn for that location.
     */
     ...
     * @param latitude  the latitude of the location in degrees.
     * @param longitude the longitude of the location in degrees (West is negative)
     * @return true if it is night at the given location and datetime. This returns true if the given datetime at the location is after the astronomical twilight dusk and before the astronomical twilight dawn.
     */
    ...
    private static SolarEquationVariables getSolarEquationVariables( final Calendar day, double longitude )
    {
        ...
        // Solar mean anomaly
        final double m = Math.toRadians( (357.5291 + 0.98560028 * (jstar - JULIAN_DATE_2000_01_01)) % CONST_360 );
